...
void svm_rank_cross_validation(const svm_problem *prob, const svm_parameter *param, int nr_fold, double *result)
{
	int i,q;
	int *fold_start;
	int l = prob->l;
...
// to avoid ranking inconsistency between models.
	...
	svm_eval_list(prob->y,target,query_set,l,result);
	free(fold_start);
	free(count);
	free(start);
	free(perm);
...
void svm_eval_list(double *label, double *target, int *query, int l, double *result_ret)
{
	int q,i,j,k;
	int nr_query;
	int *start = NULL;
